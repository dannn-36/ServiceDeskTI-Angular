...
const sampleUsers = [
    { id: 1, nombre: 'Carlos Mendoza', correo: 'carlos.mendoza@empresa.com', rol: 'EndUser', departamento: 'Ventas', estado: 'activo', ultimo_acceso: '2024-01-15 10:30' },
    { id: 2, nombre: 'Ana López', correo: 'ana.lopez@empresa.com', rol: 'Agente', departamento: 'TI', estado: 'activo', ultimo_acceso: '2024-01-15 11:45' },
    { id: 3, nombre: 'María González', correo: 'maria.gonzalez@empresa.com', rol: 'EndUser', departamento: 'Contabilidad', estado: 'activo', ultimo_acceso: '2024-01-15 09:15' },
...
const sampleTickets = [
    ...
    { id: 'TK-1247', titulo: 'Error en aplicación contable', usuario: 'María González', categoria: 'Software', estado: 'en-progreso', prioridad: 'media', fecha: '2024-01-15 08:45' },
...
const sampleAuditLog = [
    { usuario: 'Admin Sistema', accion: 'Inicio de sesión', detalle: 'Acceso desde IP 192.168.1.100', fecha: '2024-01-15 11:45' },
    { usuario: 'Ana López', accion: 'Ticket asignado', detalle: 'TK-1248 asignado a Ana López', fecha: '2024-01-15 11:30' },
...
function showSection(section, element) {
    ...
    else console.warn(`⚠️ No se encontró la sección: ${section}-section`);
  ...
  switch (section) {
    ...
    default:
        ...
        function getRoleBadgeClass(rol) {
            const classes = {
                'EndUser': 'bg-blue-100 text-blue-800',
        ...
        function showCreateUserModal() {
            showNotification('Modal de creación de usuario en desarrollo', 'info');
        }
...
function editUser(id) {
  ...
  document.getElementById("userRoleEdit").value = user.rol;
  document.getElementById("userDeptEdit").value = user.departamento;
...
document.getElementById("editUserForm").addEventListener("submit", function(event) {
  ...
  if (currentEditUserId === null) return alert("No hay usuario seleccionado para editar.");
  const user = sampleUsers.find(u => u.id === currentEditUserId);
  if (!user) return alert("Usuario no encontrado.");
  user.nombre = document.getElementById("userNameEdit").value.trim();
  user.correo = document.getElementById("userEmailEdit").value.trim();
  user.rol = document.getElementById("userRoleEdit").value;
  user.departamento = document.getElementById("userDeptEdit").value;
  renderUsers();
  closeEditUserModal();
...
// ✅ Ahora van fuera del logout, como funciones globales
...
function showProfileModal() {
    try {
        // Si currentUser no está definido por alguna razón, lo definimos con un fallback
        if (typeof window.currentUser === 'undefined' || !window.currentUser) {
            window.currentUser = {
                name: document.getElementById('profileName')?.textContent?.trim() || 'Admin Sistema',
                email: 'admin@empresa.com'
...
function saveCategoryTimes() {
    ...
    // Aquí puedes agregar tu llamada a backend, por ejemplo con fetch o axios
...
// Devuelve color según rol
function getRoleColor(role) {
    switch (role.toLowerCase()) {
        case 'enduser': return 'bg-blue-100 text-blue-800';
...
// Funciones simuladas para respaldo y restauración
...
// Placeholder para funciones de acción
...
function deleteUser(id) { alert('Eliminar usuario ID: ' + id); }
 ...
 // Guardar tiempos definidos
  ...
  // Cargar tiempos guardados al abrir configuración
        ...
        <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">${ticket.id} • ${ticket.usuario}</p>
                </div>
            </div>
...
// Función para asignar emoji según categoría
function getCategoryIcon(categoria) {
    const icons = {
        'Hardware': '🖥️',
        'Software': '💾',
    ...
    return icons[categoria] || '🎫'; // Emoji por defecto si no encuentra categoría
}
...
// Función para asignar un emoji según el tipo de acción
function getAuditIcon(accion) {
    ...
    return icons[accion] || '📝'; // Emoji por defecto si no está definido
}

// Cargar registros de auditoría con emojis
...
function createUser(event) {
  ...
  const dept = document.getElementById('userDept').value; // 🔹 Departamento seleccionado

  // 🔹 Fecha + hora actual formateada
// 🔹 Generar fecha y hora con formato: YYYY-MM-DD HH:mm
const now = new Date();
...
const fechaHora = `${year}-${month}-${day} ${hours}:${minutes}`;
  ...
  const newUser = {
    ...
    departamento: dept,
    estado: 'activo',
    ultimo_acceso: fechaHora // 🔹 Incluye hora
  };
  ...
  document.getElementById('userRole').value = 'EndUser';
